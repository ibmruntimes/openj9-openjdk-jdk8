...
class NativeCipherBlockChaining extends FeedbackCipher  {
    ...
    static {
        ...
        for (int i = 0; i < numContexts; i++) {
            contexts[i] = NativeCrypto.CBCCreateContext(0/*nativeBuffer*/, 0/*nativeBuffer2*/);
            avStack.push(i);
        }
    }
    ...
    synchronized static long getContext(NativeCipherBlockChaining cipher) {
        ...
        if (avStack.isEmpty()) {
            cipher.ctxIndx = -1;
            cipher.nativeContext = NativeCrypto.CBCCreateContext(0/*nativeBuffer*/, 0/*nativeBuffer2*/);
        } else {
